...
```{r}
scq <- read.csv("/Users/jojohu/Documents/Qlab/blast_online_data/QLABBLASTProject-SCQScores_DATA_2021-08-04_0056.csv")
...
```{r}
scq_smile <- read.csv("/Volumes/data/projects/smile/data/social_cognition/SMILE SCQ Lifetime_February 23, 2023_14.24.csv", stringsAsFactors = F)
name <- read.csv("/Volumes/data/projects/smile/data/demo/MINDSPLASH-Smilefirstname_DATA_2023-02-23_1629.csv", stringsAsFactors = F)
scq_complete <- read.csv("/Volumes/data/projects/smile/data/social_cognition/MINDSPLASH-Smiledatacheck_DATA_2023-02-23_1631.csv")
...
# Get rid of the responses that didn't have full responses
scq_smile <-
  scq_smile %>%
  filter(scq_smile$`40` != "")
```
...
missing_response <- setdiff(scq_logged, scq_smile$record_id)
```
...
scq_total <-
  scq_total %>%
  dplyr::select("record_id", "Recorded Date", as.character(1:40), "scq_total", "scq_rsid", "scq_cd", "scq_rrspb")
...
problem <- 
  as.data.frame(dplyr::bind_rows(duplicated_response, as.data.frame(missing_response))) %>%
  mutate(missing_response = as.character(missing_response)) %>%
  mutate(participant_id = coalesce(record_id, missing_response)) %>%
  dplyr::select(participant_id, n)

write.csv(problem, "/Users/jojohu/Downloads/smile_scq_problem.csv")
```
