...
adult_demo <- adult_demo[,c("record_id", "group", "participant_gender", "participant_dob")]
...
```{r, include = F}
scq <- read.csv("/Volumes/data/projects/smile/assessments/scq/scq_scores_smile_edited.csv")
...
scq<-
  scq %>% 
  filter(str_detect(record_id, "\\S+[:digit:]$")) %>%
...
## Temporary: read in KBIT scores (To Do: check if KBIT scores are correct)
```{r}
kbitPre <- read.csv("/Users/jojohu/Downloads/all_kbit (pre-Qualtrics) - SMILE.csv")
kbitPost <- read.csv("/Users/jojohu/Downloads/kbit_raw_score_qualtrics.csv")


kbitPre <- kbitPre[,c("ID", "First.Score", "X")]
colnames(kbitPre) <- c("part_id", "kbit_raw", "kbit_std")

kbitPost<-
  kbitPost[,c("part_id", "kbit_raw", "kbit_std", "kbit_age_year", "kbit_age_month")] %>%
  mutate(part_id = str_extract(part_id, "(smile|blast)_\\S+_[:digit:]+")) %>%
  filter(str_detect(part_id, "smile")) %>%
  arrange(part_id) %>%
  filter(part_id != "smile_c_300")

kbitPost[which(kbitPost$part_id == "smile_0_022"), "part_id"] <- "smile_c_022"

kbitPre <- 
  kbitPre %>%
  filter(str_detect(part_id, "smile_c_[:digit:]+")) %>%
  mutate(kbit_raw = as.numeric(as.character(kbit_raw)),
         kbit_std = as.numeric(as.character(kbit_std)))

kbit <- dplyr::bind_rows(kbitPre, kbitPost)

# Just a temp solution
kbit_manual <- read.csv("/Users/jojohu/Downloads/kbit_manual.csv")

kbit_manual$kbit_std <- coalesce(as.numeric(as.character(kbit_manual$kbit_std)), as.numeric(as.character(kbit_manual$kbit_std2)))
...
kbit <- 
  ...
  dplyr::summarise(kbit_raw = mean(kbit_raw, na.rm = T),
                   kbit_std = mean(kbit_std, na.rm = T)) %>%
  mutate(part_id = Participant.Public.ID)
  

ipad <- merge(ipad, kbit, by = c("part_id"), all.x = T)
```
...
# Data filtered: Only TD and ASD included
```{r}
...
# write.csv(demo_wide, "/Users/jojohu/Downloads/demo_wide.csv")
```
...
# write.csv(gender_wide, "/Users/jojohu/Downloads/gender_wide_td_asd.csv")
```
...
# Descriptive Analyses for Assessment Data
```{r}
immRe %>%
  dplyr::select(Participant.Public.ID, group, pvt_std_score, pvt_national_percentile, orr_std_score, orr_national_percentile, scq_total, mite_total_score, kbit_raw, kbit_std) %>%
  distinct(.) %>%
  arrange(Participant.Public.ID) %>%
  group_by(group) %>%
  dplyr::summarise(across(c("pvt_std_score", "pvt_national_percentile", "orr_std_score", "orr_national_percentile", "scq_total", "mite_total_score", "kbit_std"), list(mean = ~ mean(., na.rm = TRUE), SD = ~ sd(., na.rm = TRUE), N = ~ sum(!is.na(.))))) %>%
  slice(match(c("TD", "ASD"), group)) %>%
  mutate_if(is.numeric, round, 2)
  
ast_scores <- 
  immRe %>%
  dplyr::select(Participant.Public.ID, group, pvt_std_score, orr_std_score, scq_total, mite_total_score, kbit_std) %>%
  distinct(.) 
  
t.test(pvt_std_score~group, data = ast_scores, paired = F) 
t.test(orr_std_score~group, data = ast_scores, paired = F) 
t.test(scq_total~group, data = ast_scores, paired = F) 
t.test(kbit_std~group, data = ast_scores, paired = F) 

# write.csv(ast_scores, "/Users/jojohu/Downloads/ast_scores.csv")
```
...
# Uncomment to Change week 3 trials to retention
```{r}
# blockRe[which(blockRe$Participant.Public.ID %in% thirdID &
#                 blockRe$display %in% c("post-test", "post-testV") &
#                 blockRe$test_time == "long_retention"),]$test_time <- "retention"
```
...
simple_att1 <- glmer(corr ~ group*social_cond + immCorr + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                   data = blockRe[which(blockRe$test_time == "attainment"),],
                   family = binomial,
                   contrasts = list(group = c(-0.5, 0.5), 
                                    social_cond = c(-0.5, 0.5)),
                   control = glmerControl(optimizer = "bobyqa"))
...
simple_attTD <- glmer(corr ~ social_cond + immCorr + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
...
simple_attASD <- glmer(corr ~ social_cond + immCorr + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
...
```{r}
simple_ret <- glmer(corr ~ group*social_cond + immCorr + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
...
simple_retTD <- glmer(corr ~ social_cond + immCorr + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
...
simple_retASD <- glmer(corr ~ social_cond + immCorr + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
...
simple_immRetentionASD <- glmer(corr ~ social_cond*test_three + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                          data = immRetentionASD,
                          family = binomial,
                          contrasts = list(
                          test_three = c(-0.5, 0.5),
                          social_cond = c(-0.5, 0.5)
...
summary(simple_immRetentionASD)
...
simple_blockReTD2 <- glmer(corr ~ social_cond*test_three*scaled_age + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                   data = blockReTD,
                   family = binomial,
                   contrasts = list(social_cond = c(-0.5, 0.5),
                                    test_three = c(-0.5, 0.5)),
...
simple_blockReASD1 <- glmer(corr ~ social_cond*test_three + immCorr + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                   data = blockReASD,
                   family = binomial,
                   contrasts = list(social_cond = c(-0.5, 0.5),
                                    test_three = c(-0.5, 0.5)),
...
summary(simple_blockReASD1)

simple_blockReASD2 <- glmer(corr ~ social_cond*test_three + immCorr + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                   data = blockReASD[-which(blockReASD$test_time == "short_retention"),],
                   family = binomial,
                   contrasts = list(social_cond = c(-0.5, 0.5),
                                    test_three = c(-0.5, 0.5)),
                   control = glmerControl(optimizer = "bobyqa"))

summary(simple_blockReASD2)

meanAccAttRet <- 
  blockReASD[-which(blockReASD$test_time == "short_retention"),] %>%
  filter(group == "ASD") %>%
...
simple_allReASD2 <- glmer(corr ~ social_cond*test_three + 
                          (1 + social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = allReASD,
                    family = binomial,
                    contrasts = list(
...
# ggsave(paste0(output_path, "att_ret_by_group.png"), bg="transparent", width = 25, height = 15, units = "cm")
...
ggsave(paste0(output_path, "imm_across_group.png"), bg="transparent", width = 20, height = 10, units = "cm")
```


# Plot Mean of Attainment and Retention in ASD
```{r}
allRe %>%
    # filter(word_type == word_class1 | word_type == word_class2) %>%
    filter(group == "ASD") %>%
  ...
  # scale_pattern_spacing_discrete(range = c(0.025, 0.05)) + 
    ...
    coord_cartesian(ylim = c(0.5, 0.87)) +
...
ggsave(paste0(output_path, "retASD_attlr.png"), bg="transparent", width = 20, height = 15, units = "cm")    
```
...
allRe %>%
  ...
  # scale_pattern_spacing_discrete(range = c(0.025, 0.05)) + 
    ...
    coord_cartesian(ylim = c(0.5, 0.79)) +
...
ggsave(paste0(output_path, "retGroup_attlr.png"), bg="transparent", width = 20, height = 10, units = "cm")  
```
...
allRe %>%
   ...
   # guides(fill = guide_legend(title="Social Condition")) +
    ...
    coord_cartesian(ylim = c(0.50, 0.79)) +
    geom_segment(data = data.frame(test_three = "Long-Term Retention", group = "ASD", social_cond = "Direct Address"),
    ...
    #           data = data.frame(test_three = "Long-Term Retention", group = "ASD", social_cond = "Direct Address"),
    #           aes(x = 2, y = 0.71, label = paste0("?"))) +
    geom_text(size = 7, color = "black", 
              data = data.frame(test_three = "Long-Term Retention", group = "ASD", social_cond = "Direct Address"),
              aes(x = 1.5, y = 0.76, label = paste0("†"))) +
  geom_text(size = 11, color = "black", 
              data = data.frame(test_three = "Short-Term Retention", group = "ASD", social_cond = "Direct Address"),
              aes(x = 1.5, y = 0.78, label = paste0("*"))) +
  geom_segment(data = data.frame(test_three = "Short-Term Retention", group = "ASD", social_cond = "Direct Address"),
...
```{r}
ggsave(paste0(output_path, "att_ret_by_time.png"), bg="transparent", width = 30, height = 10, units = "cm")
```
...
gaze_child %>%
  # filter(group == "ASD") %>%
  filter(look_object == "speaker" | look_object == "listener" | look_object == "away" | look_object == "cannot_tell") %>%
  arrange(group) %>%  # First sort by val. This sort the dataframe but NOT the factor levels
  mutate(group = factor(group, levels=c("TD", "ASD")),
         look_object = factor(look_object, levels=c("speaker", "listener", "away", "cannot_tell"))) %>%
  mutate(social_cond = dplyr::recode(social_cond, Direct = "Direct Address"),
         look_object = dplyr::recode(look_object, cannot_tell = "Unclear")) %>%
  ...
  ggplot(aes(x = look_object, y = mean_freq, pattern = social_cond,  fill = group)) +
    geom_col_pattern(aes(pattern = social_cond),
                     stat = "summary",
                     fun = "mean",
                     width = 0.7,
    ...
    geom_point(color = "black",
               ...
               fun = "mean",
               show.legend = FALSE) +
  facet_grid(~group, labeller = (labeller(look_object = labsLook))) +
  labs(y = "Mean Proportion of Looks (%)") +
  styling +
  theme(axis.title.x = element_blank())


ggsave(paste0(output_path, "looks_td_asd.png"), bg="transparent", width = 40, height = 15, units = "cm")
```


## Plot Eye Gaze Proportions in each group
```{r}
labsLook <- c("Unclear", "Speaker", "Listener")
names(labsLook) <- c("cannot_tell", "speaker", "listener")
...
gaze_child %>%
  filter(group == "ASD") %>%
  filter(look_object == "speaker" | look_object == "listener") %>%
  arrange(group) %>%  # First sort by val. This sort the dataframe but NOT the factor levels
  mutate(group = factor(group, levels=c("TD", "ASD")),
  ...
  mutate(lower_ci = mean_freq - ci,
         upper_ci = mean_freq + ci) %>%
  arrange(group, social_cond, look_object) %>%
  ggplot(aes(x = social_cond, y = mean_freq, fill = social_cond)) +
...
ggsave(paste0(output_path, "looks_td.png"), bg="transparent", width = 40, height = 25, units = "cm")
```
...
plot_gazeAcc("ASD", "immediate")
plot_gazeAcc("ASD", "attainment")
plot_gazeAcc("ASD", "retention")
...
ggsave("/Users/jojohu/Documents/Splash/beh_analysis/results/gaze_acc_oh_asd.png", width = 15, height = 15, units = "cm")
```
