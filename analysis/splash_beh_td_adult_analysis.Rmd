...
smile_demo <- read.csv("/Users/jojohu/Documents/Splash/beh_analysis/demo/MINDSPLASH-Smilechilddob_DATA_2023-02-24_0645.csv")
...
adult_demo <- read.csv("/Users/jojohu/Documents/Splash/beh_analysis/demo/MINDSPLASH-Smilemedaladultdob_DATA_2023-03-16_1448.csv")

adult_group <- read.csv("/Users/jojohu/Documents/Splash/beh_analysis/demo/MINDSPLASH-Splashmedaladultdemo_DATA_LABELS_2023-03-16_1445.csv")

newCol <- str_extract(colnames(adult_group), "Bilingual")

colnames(adult_group) <- coalesce(newCol, colnames(adult_group))

adult_group <- adult_group[,c("Record.ID", "Bilingual")]

adult_demo <- merge(adult_demo, adult_group, by.x = c("record_id"), by.y = c("Record.ID"), all.x = T)

adult_demo$group <- "Adult"

adult_demo$participant_gender <- as.character(adult_demo$participant_gender)

adult_demo$participant_gender <- plyr::revalue(adult_demo$participant_gender, 
                                          c("1" = "M",
                                            "2" = "F"))

adult_demo <- adult_demo[,c("record_id", "group", "participant_gender", "participant_dob", "Bilingual")]
adult_demo$short_id <- str_remove(adult_demo$record_id, "smile_a_")

smile_demo <- 
  dplyr::bind_rows(smile_demo, adult_demo) %>%
  distinct(.)
```
...
ipad <- 
  ...
  map_dfr(`[`, c("PIN", "Inst", "Uncorrected.Standard.Score",	"Age.Corrected.Standard.Score",	"National.Percentile..age.adjusted.", "DateFinished"))  %>%
  dplyr::bind_rows()

ipad <- ipad[,c("PIN", "Inst", "Uncorrected.Standard.Score",	"Age.Corrected.Standard.Score",	"National.Percentile..age.adjusted.", "DateFinished")]
colnames(ipad) <- c("part_id", "ast", "raw_score", "std_score", "national_percentile", "date")
...
ipadL <- melt(ipad, id.vars = c("part_id", "ast", "date"))

ipadL$date <- as.Date(ipadL$date)
...
# smile_c_300 accidentally did the iPad tasks twice. Take the first time score.
if(nrow(ipadL[which(ipadL$date == "2022-07-15" & ipadL$part_id == "smile_c_300"),]) > 0) {
  ipadL <- ipadL[-which(ipadL$date == "2022-07-15" & ipadL$part_id == "smile_c_300"),]
}
...
mite <- read.csv("/Users/jojohu/Documents/Splash/beh_analysis/social_cognition/Mind in the Eyes_February 23, 2023_15.46.csv", stringsAsFactors = F)
...
mite <- mite[which(str_detect(mite$record_id, "smile_c_[:alnum:]")),]
```

## Change MITE ID from smile_c_384 to smile_c_449 and smile_c_cor384 to smile_c_384 due to administration error
```{r}
mite[which(mite$record_id == "smile_c_384"), "record_id"] <- "smile_c_449"
mite[which(mite$record_id == "smile_c_cor384"), "record_id"] <- "smile_c_384"

mite[which(mite$record_id == "smile_c_384" | mite$record_id == "smile_c_449"), "record_id"]
```
...
# Reset child part_id
behDF[which(as.numeric(as.character(behDF$Participant.Public.ID)) < 800 & !str_detect(behDF$Participant.Public.ID, "smile")),]$Participant.Public.ID <- 
  paste0("smile_c_", behDF[which(as.numeric(as.character(behDF$Participant.Public.ID)) < 800 & !str_detect(behDF$Participant.Public.ID, "smile")),]$Participant.Public.ID)
# Reset adult part_id
behDF[which(as.numeric(as.character(behDF$Participant.Public.ID)) > 799 & !str_detect(behDF$Participant.Public.ID, "smile")),]$Participant.Public.ID <- 
  paste0("smile_a_", behDF[which(as.numeric(as.character(behDF$Participant.Public.ID)) > 799 & !str_detect(behDF$Participant.Public.ID, "smile")),]$Participant.Public.ID)
...
behDF[which(behDF$Participant.Public.ID == "smileCoo8"), "Participant.Public.ID"] <- "smile_c_008"

unique(behDF$Participant.Public.ID)
```


# Merge Behavioral with Demo Data
```{r}
...
behDF[which(str_detect(behDF$Participant.Public.ID, "smile_a")),]$group <- "adult"

behDF[which(as.numeric(as.character(str_extract(behDF$short_id, "[:digit:]+"))) > 799 & as.numeric(as.character(str_extract(behDF$short_id, "[:digit:]+"))) < 900), "modality"] <- "auditory"

behDF[which(as.numeric(as.character(str_extract(behDF$short_id, "[:digit:]+"))) < 799 | as.numeric(as.character(str_extract(behDF$short_id, "[:digit:]+"))) > 899), "modality"] <- "visual"

behDF %>%
  dplyr::select(Participant.Public.ID, group, modality) %>%
  distinct(.) %>%
  group_by(Participant.Public.ID, group, modality) %>%
  dplyr::summarise(n = n())
...
# Data filtered: Only TD and SPLASH Adult included
```{r}
# immRe <- immRe[which(immRe$Participant.Public.ID %in% demo_wide$Participant.Public.ID),]
# behDF <- 
  behDF %>%
  filter(group == "TD" | group == "adult") %>%
  filter(short_id < 800 | short_id > 899)
...
immRe <- immRe[,c("Participant.Public.ID", "group", "participant_gender", "age_month", "age_year", 
                  "Response", "video_file", "UTC.Date", "Event.Index", "Spreadsheet", "Spreadsheet.Row", 
                  "Screen.Number", "Task.Name", "Experiment.Version", 
                  "Tree.Node.Key", "Participant.Viewport.Size",
...
ast_groupTD <- 
  immRe %>%
  dplyr::select(Participant.Public.ID, group, age_group, pvt_national_percentile, orr_national_percentile) %>%
  distinct(.) %>%
  group_by(group) %>%
  filter(group == "TD") %>%
  mutate(pvt_group = c("pvt_higher", "pvt_lower")[1 + (pvt_national_percentile <= median(pvt_national_percentile, na.rm = T))],
         orr_group = c("orr_higher", "orr_lower")[1 + (orr_national_percentile <= median(orr_national_percentile, na.rm = T))])

ast_groupTD %>%
  group_by(age_group, pvt_group) %>%
  dplyr::summarise(n = length(unique(Participant.Public.ID)))

ast_groupTD %>%
  group_by(age_group, orr_group) %>%
  dplyr::summarise(n = length(unique(Participant.Public.ID)))
...
demo_wide <- rbind(demo_wideAd, demo_wideTD)
```


# Data filtered: Only TD and Adult included
```{r}
# immRe <- immRe[which(immRe$Participant.Public.ID %in% demo_wide$Participant.Public.ID),]
immRe <- 
  immRe %>%
...
# write.csv(demo_wide, "/Users/jojohu/Downloads/demo_wide_td_adult.csv")
```
...
# write.csv(gender_wide, "/Users/jojohu/Downloads/gender_wide_td_adult.csv")
```
...
immRe$group <- factor(immRe$group, levels = c("adult", "TD"))
...
immReAd <- immRe[which(immRe$group == "adult"),]

immReTD$social_cond <- factor(immReTD$social_cond, levels = c("front", "side"))
immReTD <- immReTD[order(immReTD$social_cond),]

immReAd$social_cond <- factor(immReAd$social_cond, levels = c("front", "side"))
immReAd <- immReAd[order(immReAd$social_cond),]
...
simple_immReTD2 <- glmer(corr ~ social_cond*scaled_age + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                   data = immReTD,
                   family = binomial,
                   contrasts = list(social_cond = c(-0.5, 0.5)),
                   control = glmerControl(optimizer = "bobyqa"))
...
## Immediate Recall Adult Analysis
```{r}
immReAd$scaled_age_ad <- scale(immReAd$age_year)

immReAd <- immReAd[which(as.numeric(str_extract(immReAd$Participant.Public.ID, "(?<=smile_a_)\\S+")) > 899),]

simple_immReAd <- glmer(corr ~ social_cond + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                   data = immReAd,
                   family = binomial,
                   contrasts = list(social_cond = c(-0.5, 0.5)),
                   control = glmerControl(optimizer = "bobyqa"))

summary(simple_immReAd)

simple_immReAd2 <- glmer(corr ~ social_cond*scaled_age_ad + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                   data = immReAd,
                   family = binomial,
                   contrasts = list(social_cond = c(-0.5, 0.5)),
                   control = glmerControl(optimizer = "bobyqa"))

summary(simple_immReAd2)

simple_immReAd3 <- glmer(corr ~ social_cond*word_type + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                   data = immReAd,
                   family = binomial,
                   contrasts = list(social_cond = c(-0.5, 0.5),
                                    word_type = c(-0.5, 0.5)),
...
wrap_t(meanAccImm, "adult", "front")
wrap_t(meanAccImm, "TD", "side")
wrap_t(meanAccImm, "adult", "side")
...
moreRe[which(moreRe$Participant.Public.ID == "smile_c_904_1"),]$Participant.Public.ID <- "smile_c_904"
...
blockRe$UTC.Date <- as.Date(blockRe$UTC.Date)
...
# Merge blockRe with Immediate learning accuracy
 immCorr <- 
   immRe[,c("Participant.Public.ID", "story", "corr")] %>%
   mutate(immCorr = corr) %>%
   dplyr::select(-one_of("corr")) %>%
   distinct(.)

blockRe <- merge(blockRe, immCorr, by = c("Participant.Public.ID", "story"), all.x = T)
...
blockRe$group <- factor(blockRe$group, levels = c("adult", "TD"))
...
blockReAd <- blockRe[which(blockRe$group == "adult"),]
...
contrasts(blockReAd$test_time) = three_helmert

blockReTD$social_cond <- factor(blockReTD$social_cond, levels = c("Direct", "Overhearing"))
blockReTD <- blockReTD[order(blockReTD$social_cond),]
...
simple_att <- glmer(corr ~ group*social_cond + immCorr + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
...
simple_att2 <- glmer(corr ~ group*social_cond*scaled_age + immCorr + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                   data = blockRe[which(blockRe$test_three == "attainment"),],
                   family = binomial,
                   contrasts = list(group = c(-0.5, 0.5), 
                                    social_cond = c(-0.5, 0.5)),
...
m1_attOld <- 
  glmer(corr ~ social_cond + immCorr + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = blockRe[which(blockRe$test_three == "attainment" & 
...
scale_var <-
  function(df, test_time, group) {
    ...
    scale(df[which(df$test_three == test_time & df$group == group),]$pvt_std_score)
      ...
      df[which(df$test_three == test_time & df$group == group), "scaled_pvt_raw"] <-
    scale(df[which(df$test_three == test_time & df$group == group),]$pvt_raw_score)
    ...
    scale(df[which(df$test_three == test_time & df$group == group),]$mite_total_score)
      ...
      df[which(df$test_three == test_time & df$group == group), "scaled_orr_std"] <-
    scale(df[which(df$test_three == test_time & df$group == group),]$orr_std_score)
      
      df[which(df$test_three == test_time & df$group == group), "scaled_orr_raw"] <-
    scale(df[which(df$test_three == test_time & df$group == group),]$orr_raw_score)
...
# What explains the age effect in attainment overhearing > direct address in attainment?
simple_attTD1.1.1 <- glmer(corr ~ social_cond*scaled_age*scaled_pvt_raw*scaled_orr_raw +  (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = blockRe[which(blockRe$test_three == "attainment" & blockRe$group == "TD"),],
                    family = binomial,
                    contrasts = list(
                    social_cond = c(-0.5, 0.5)
...
summary(simple_attTD1.1.1)

simple_attTD1.1.2 <- glmer(corr ~ social_cond*scaled_age*scaled_orr_raw +  (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = blockRe[which(blockRe$test_three == "attainment" & blockRe$group == "TD"),],
                    family = binomial,
                    contrasts = list(
                    social_cond = c(-0.5, 0.5)
...
storyAccAttTD <- 
  ...
  group_by(Participant.Public.ID, group, test_time, social_cond, age_group, age_year, pvt_raw_score, pvt_std_score, scaled_pvt_std, orr_raw_score, orr_std_score, scaled_orr_std) %>%
  dplyr::summarise(corr = mean(corr, na.rm = T)) %>%
  spread(social_cond, corr)

storyAccAttTD$scaled_direct <-  scale(storyAccAttTD$Direct)
storyAccAttTD$scaled_overhearing <-  scale(storyAccAttTD$Overhearing)
storyAccAttTD$diff <- storyAccAttTD$scaled_direct - storyAccAttTD$scaled_overhearing


simple_attTD1.1.1 <- lm(diff ~ orr_raw_score,data = storyAccAttTD)
...
```{r}
simple_attTD1.2 <- glmer(corr ~ social_cond*age_group +  (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = blockRe[which(blockRe$test_three == "attainment" & blockRe$group == "TD"),],
                    family = binomial,
                    contrasts = list(
                    social_cond = c(-0.5, 0.5),
...
summary(simple_attTD1.2)



simple_attTD1.2 <- glmer(corr ~ social_cond*test_three +  (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = blockRe[which(blockRe$group == "TD" & blockRe$age_group == "older"),],
                    family = binomial,
                    contrasts = list(
                    social_cond = c(-0.5, 0.5),
                    age_group = c(-0.5, 0.5),
...
simple_attTD2 <- glmer(corr ~ social_cond*scaled_age + immCorr +  (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = blockRe[which(blockRe$test_three == "attainment" & blockRe$group == "TD"),],
                    family = binomial,
                    contrasts = list(
                    social_cond = c(-0.5, 0.5)
...
blockRe %>%
  ...
  #   fun.y = "mean"
  # ) +
  coord_cartesian(,c(0.65, 0.75)) +
  styling

blockRe %>%
  # filter(group == "TD") %>%
  filter(test_three == "attainment") %>%
  group_by(Participant.Public.ID, group, test_three, social_cond) %>%
  dplyr::summarise(corr = mean(corr, na.rm = T)) %>%
  group_by(group, test_three, social_cond) %>%
  dplyr::summarise(mean_acc = mean(corr, na.rm = T))
```
...
## Attainment Social Condition by Age Interaction Effect in TD
```{r}
library(ggbeeswarm)
...
blockRe %>%
    ...
    group_by(Participant.Public.ID, group, age_group, social_cond, test_three) %>%
    # dplyr::summarise(mean = mean(corr, na.rm = TRUE)) %>%
    summarySEwithin(., measurevar = "corr", betweenvars = c("group", "age_group"),  withinvars = c("social_cond", "test_three"), idvar= "Participant.Public.ID", na.rm=T, conf.interval=.95) %>%
    mutate(lower_ci = corr - ci,
           upper_ci = corr + ci) %>%
    ggplot(aes(x = age_group, y = corr, group = social_cond,  fill = social_cond)) +
    ...
    scale_fill_manual(values=c(cbPalette[2], cbPalette[8])) +
...
## Attainment Adults Analysis
```{r}
blockRe <- scale_var(blockRe, "attainment", "adult")

blockRe <- blockRe[which(as.numeric(str_extract(blockRe$Participant.Public.ID, "(?<=smile_a_)\\S+")) > 899),]

simple_attAd <- glmer(corr ~ social_cond + (1|Participant.Public.ID),
                    data = blockRe[which(blockRe$test_time == "retention"),],
                    family = binomial,
                    contrasts = list(
                    social_cond = c(-0.5, 0.5)
                   ),
...
summary(simple_attAd)

simple_attAd1 <- glmer(corr ~ social_cond + immCorr + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = blockRe[which(blockRe$test_three == "attainment" & blockRe$group == "adult"),],
                    family = binomial,
                    contrasts = list(
                    social_cond = c(-0.5, 0.5)
                   ),
...
summary(simple_attAd1)

simple_attAd2 <- glmer(corr ~ social_cond*scaled_age + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = blockRe[which(blockRe$test_three == "attainment" & blockRe$group == "adult"),],
                    family = binomial,
                    contrasts = list(
                    social_cond = c(-0.5, 0.5)
...
summary(simple_attAd2)

simple_attAd3 <- glmer(corr ~ social_cond*scaled_age*word_type + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = blockRe[which(blockRe$test_three == "attainment" & blockRe$group == "adult"),],
                    family = binomial,
                    contrasts = list(
                    social_cond = c(-0.5, 0.5),
...
summary(simple_attAd3)
```

## Attainment Social Condition * Word_Type * Age Interaction Effect in TD
```{r}
library(ggbeeswarm)
...
blockRe %>%
  filter(group == "adult" & test_three == "attainment") %>%
...
wrap_t(meanAccAtt, "adult", "Direct")
wrap_t(meanAccAtt, "adult", "Overhearing")
...
t.test(meanAccAttTDOld$Overhearing, meanAccAttTDOld$Direct, paired = T, alternative = "greater")
...
simple_ret1 <- glmer(corr ~ group*social_cond + immCorr + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                   data = blockRe[which(blockRe$test_three == "retention"),],
                   family = binomial,
                   contrasts = list(group = c(-0.5, 0.5), 
                                    social_cond = c(-0.5, 0.5)),
...
simple_retTD1 <- glmer(corr ~ social_cond + immCorr + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = blockRe[which(blockRe$test_three == "retention" & blockRe$group == "TD"),],
                    family = binomial,
                    contrasts = list(
                    social_cond = c(-0.5, 0.5)
...
summary(simple_retTD1)


simple_retTD2.1 <- glmer(corr ~ social_cond*scaled_age + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = blockRe[which(blockRe$test_three == "retention" & blockRe$group == "TD"),],
                    family = binomial,
                    contrasts = list(
                    social_cond = c(-0.5, 0.5)
...
summary(simple_retTD2.1)

simple_retTD2 <- glmer(corr ~ social_cond*age_group + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = blockRe[which(blockRe$test_three == "retention" & blockRe$group == "TD"),],
                    family = binomial,
                    contrasts = list(
                    social_cond = c(-0.5, 0.5),
...
## Retention Adult Analysis
```{r}
blockRe <- scale_var(blockRe, "retention", "adult")

simple_retAd <- glmer(corr ~ social_cond + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = blockRe[which(blockRe$test_three == "retention" & blockRe$group == "adult"),],
                    family = binomial,
                    contrasts = list(
                    social_cond = c(-0.5, 0.5)
                   ),
...
summary(simple_retAd)

simple_retAd1 <- glmer(corr ~ social_cond + immCorr + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = blockRe[which(blockRe$test_three == "retention" & blockRe$group == "adult"),],
                    family = binomial,
                    contrasts = list(
                    social_cond = c(-0.5, 0.5)
...
summary(simple_retAd1)

simple_retAd2 <- glmer(corr ~ social_cond*scaled_age + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = blockRe[which(blockRe$test_three == "retention" & blockRe$group == "adult"),],
                    family = binomial,
                    contrasts = list(
                    social_cond = c(-0.5, 0.5)
...
summary(simple_retAd2)

simple_retAd3 <- glmer(corr ~ social_cond*scaled_age*word_type + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = blockRe[which(blockRe$test_three == "retention" & blockRe$group == "adult"),],
                    family = binomial,
                    contrasts = list(
                    social_cond = c(-0.5, 0.5),
...
wrap_t(meanAccRet, "adult", "Direct")
wrap_t(meanAccRet, "adult", "Overhearing")
...
t.test(corr~social_cond, paired = T, data = meanAccRetAd, alternative = "less")
...
# write.csv(allRe, "/Users/jojohu/Documents/Splash/beh_analysis/results/allRe_data.csv", row.names = F)
```
...
allRe$group <- factor(allRe$group, levels = c("adult", "TD"))
...
allReAd <- allRe[which(allRe$group == "adult"),]
...
immRetentionAd <- immRetention[which(immRetention$group == "adult"),]

simple_immRetention <- glmer(corr ~ group*social_cond*test_three + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                          data = immRetention,
                          family = binomial,
                          contrasts = list(
                          group = c(-0.5, 0.5),
...
summary(simple_immRetention)


simple_immRetentionTD <- glmer(corr ~ social_cond*test_three + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                          data = immRetentionTD,
                          family = binomial,
                          contrasts = list(
                          test_three = c(-0.5, 0.5),
...
summary(simple_immRetentionTD)

simple_immRetentionAd <- glmer(corr ~ social_cond*test_three + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                          data = immRetentionAd,
                          family = binomial,
                          contrasts = list(
                          test_three = c(-0.5, 0.5),
...
# Attainment vs. Retention Analysis
```{r}
simple_blockRe1 <- glmer(corr ~ group*social_cond*test_three + immCorr + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                   data = blockRe,
                   family = binomial,
                   contrasts = list(group = c(-0.5, 0.5), 
                                    social_cond = c(-0.5, 0.5),
...
summary(simple_blockRe1)

simple_blockRe2 <- glmer(corr ~ group*social_cond + immCorr + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                   data = blockRe[-which(blockRe$test_time == "short_retention"),],
                   family = binomial,
                   contrasts = list(group = c(-0.5, 0.5), 
                                    social_cond = c(-0.5, 0.5),
                                    test_three = c(-0.5, 0.5)),
...
summary(simple_blockRe2)

simple_blockReTD1 <- glmer(corr ~ social_cond*test_three*scaled_age + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                   data = blockReTD,
                   family = binomial,
                   contrasts = list(social_cond = c(-0.5, 0.5),
                                    test_three = c(-0.5, 0.5)),
...
summary(simple_blockReTD1)

simple_blockReTD2 <- glmer(corr ~ social_cond*test_three + immCorr + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                   data = blockReTD[-which(blockReTD$test_time == "short_retention"),],
                   family = binomial,
                   contrasts = list(social_cond = c(-0.5, 0.5),
                                    test_three = c(-0.5, 0.5)),
                   control = glmerControl(optimizer = "bobyqa"))

summary(simple_blockReTD2)


simple_blockReAd1 <- glmer(corr ~ social_cond*test_three + immCorr + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                   data = blockReAd,
                   family = binomial,
                   contrasts = list(social_cond = c(-0.5, 0.5),
                                    test_three = c(-0.5, 0.5)),
...
summary(simple_blockReAd1)

simple_blockReAd2 <- glmer(corr ~ social_cond*test_three + immCorr + (1+social_cond|Participant.Public.ID) + (1+social_cond|story),
                   data = blockReAd[-which(blockReAd$test_time == "short_retention"),],
                   family = binomial,
                   contrasts = list(social_cond = c(-0.5, 0.5),
                                    test_three = c(-0.5, 0.5)),
                   control = glmerControl(optimizer = "bobyqa"))

summary(simple_blockReAd2)

meanAccAttRet <- 
  blockReAd[-which(blockReAd$test_time == "short_retention"),] %>%
  filter(group == "adult") %>%
  group_by(Participant.Public.ID, group, test_three, social_cond) %>%
  dplyr::summarise(mean = mean(corr, na.rm =T))

t.test(mean ~ social_cond, data = meanAccAttRet, paired = TRUE, alternative = "less")
```
...
simple_allRe1 <- glmer(corr ~ group*social_cond + (1 + social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = allRe[-which(allRe$test_time == "short_retention"),],
                    family = binomial,
                    contrasts = list(
                    group = c(-0.5, 0.5),
                    social_cond = c(-0.5, 0.5)
...
simple_allRe3 <- glmer(corr ~ group*social_cond*test_three +
...
simple_allRe4 <- glmer(corr ~ group*social_cond*test_three*scaled_age + 
                          (1 + social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = allRe,
                    family = binomial,
                    contrasts = list(
...
simple_allReTD1 <- glmer(corr ~ social_cond*test_three + 
                          (1 + social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = allReTD[-which(allReTD$test_time == "short_retention"),],
                    family = binomial,
                    contrasts = list(
                    social_cond = c(-0.5, 0.5),
                    test_three = three_helmert
...
summary(simple_allReTD1)

simple_allReTD2 <- glmer(corr ~ social_cond*test_three + 
                          (1 + social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = allReTD,
                    family = binomial,
                    contrasts = list(
...
simple_allReTD3.1 <- glmer(corr ~ social_cond*test_three*scaled_age +
                          (1 + social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = allReTD,
                    family = binomial,
                    contrasts = list(
...
## Immediate Recall, Attainment, Retention Analysis in Adult
```{r}
simple_allReAd <- glmer(corr ~ social_cond + (1 + social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = allReAd,
                    family = binomial,
                    contrasts = list(
                    social_cond = c(-0.5, 0.5)
...
summary(simple_allReAd)


simple_allReAd2 <- glmer(corr ~ social_cond*test_three + 
                          (1 + social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = allReAd,
                    family = binomial,
                    contrasts = list(
...
summary(simple_allReAd2)

simple_allReAd3 <- glmer(corr ~ social_cond*scaled_age*test_three + 
                          (1 + social_cond|Participant.Public.ID) + (1+social_cond|story),
                    data = allReAd[which(allReAd$test_three == "attainment" | 
                                           allReAd$test_three == "retention"),],
                    family = binomial,
                    contrasts = list(
                    social_cond = c(-0.5, 0.5),
                    # test_three = three_helmert,
...
allRe %>%
    ...
    filter(!is.na(test_three)) %>%
    mutate(age_group = factor(age_group, levels = c("younger", "older"))) %>%
    arrange(age_group, social_cond, test_three) %>%
    mutate(social_cond = dplyr::recode(social_cond, Direct = "Direct Address"),
           age_group = dplyr::recode(age_group, younger = "Younger Children", older = "Older Children"),
           test_three = dplyr::recode(test_three, immediate = "0 min", attainment = "Same-Day", retention = "One Week")) %>%
    group_by(Participant.Public.ID, group, age_group, social_cond, test_three) %>%
    # dplyr::summarise(mean = mean(corr, na.rm = TRUE)) %>%
    summarySEwithin(., measurevar = "corr", betweenvars = c("age_group"),  withinvars = c("social_cond", "test_three"), idvar= "Participant.Public.ID", na.rm=T, conf.interval=.95) %>%
    mutate(lower_ci = corr - ci,
           upper_ci = corr + ci) %>%
    ggplot(aes(x = age_group, y = corr, group = social_cond,  fill = social_cond)) +
    geom_bar(color = "black",
    ...
    # facet_wrap(~age_group,  labeller = (labeller(age_group = labs))) +
    scale_fill_manual(values=c(cbPalette[2], cbPalette[8])) +
    facet_grid(~test_three) +
    guides(fill = guide_legend(title = "Social Condition")) +
    labs(
        # title = "Accuracy Across All Time Points",
         x = "Social Condition",  # Change x-axis label
         y = "Accuracy (%)") +
...
ggsave(paste0(output_path, "acc_by_time_ageTD.png"), bg="transparent", width = 55, height = 20, units = "cm")
...
# Plot Social * PVT Interaction across all timepoints in TD (from multiple TD models)
```{r}
allReAST <- merge(allRe, ast_groupTD[,c("Participant.Public.ID", "pvt_group", "orr_group")], by = c("Participant.Public.ID"), all.x = T)

allReAST %>%
    filter(group == "TD") %>%
    filter(!is.na(pvt_group)) %>%
    filter(!is.na(test_three)) %>%
    filter(test_three == "attainment") %>%
    mutate(pvt_group = factor(pvt_group, levels = c("pvt_lower", "pvt_higher")),
           age_group = factor(age_group, levels = c("younger", "older"))) %>%
    arrange(pvt_group, social_cond, test_three) %>%
    mutate(social_cond = dplyr::recode(social_cond, Direct = "Direct Address"),
           pvt_group = dplyr::recode(pvt_group, pvt_lower = "Smaller Vocabulary Group", pvt_higher = "Larger Vocabulary Group"),
           # age_group = dplyr::recode(age_group, younger = "Younger Children", older = "Older Children"),
           test_three = dplyr::recode(test_three, immediate = "0 min", attainment = "Same-Day", retention = "One Week")) %>%
    group_by(Participant.Public.ID, group, pvt_group, social_cond, test_three) %>%
    # dplyr::summarise(mean = mean(corr, na.rm = TRUE)) %>%
    summarySEwithin(., measurevar = "corr", betweenvars = c("pvt_group", "age_group"),  withinvars = c("social_cond", "test_three"), idvar= "Participant.Public.ID", na.rm=T, conf.interval=.95) %>%
    mutate(lower_ci = corr - ci,
           upper_ci = corr + ci) %>%
    ggplot(aes(x = pvt_group, y = corr, group = social_cond,  fill = social_cond)) +
    geom_bar(color = "black",
              position = position_dodge(),
              width = 0.7,
              stat = "summary",
    ...
    labs(
        # title = "Accuracy Across All Time Points",
         x = "Social Condition",  # Change x-axis label
         y = "Accuracy (%)") +
    coord_cartesian(ylim = c(0.5, 0.97))  +
    styling +
    theme(axis.title.x = element_blank())

ggsave(paste0(output_path, "acc_by_pvtTD.png"), bg="transparent", width =55, height = 20, units = "cm")
...
allReAST %>%
    filter(group == "TD") %>%
    filter(!is.na(orr_group)) %>%
    filter(!is.na(test_three)) %>%
    filter(test_three == "attainment") %>%
    mutate(orr_group = factor(orr_group, levels = c("orr_lower", "orr_higher")),
           age_group = factor(age_group, levels = c("younger", "older"))) %>%
    arrange(orr_group, social_cond, test_three) %>%
    mutate(social_cond = dplyr::recode(social_cond, Direct = "Direct Address"),
           # pvt_group = dplyr::recode(pvt_group, pvt_lower = "Smaller Vocabulary Group", pvt_higher = "Larger Vocabulary Group"),
           orr_group = dplyr::recode(orr_group, orr_lower = "Lower Reading Group", orr_higher = "Higher Reading Group"),
           # age_group = dplyr::recode(age_group, younger = "Younger Children", older = "Older Children"),
           test_three = dplyr::recode(test_three, immediate = "0 min", attainment = "Same-Day", retention = "One Week")) %>%
    group_by(Participant.Public.ID, group, orr_group, social_cond, test_three) %>%
    # dplyr::summarise(mean = mean(corr, na.rm = TRUE)) %>%
    summarySEwithin(., measurevar = "corr", betweenvars = c("orr_group", "age_group"),  withinvars = c("social_cond", "test_three"), idvar= "Participant.Public.ID", na.rm=T, conf.interval=.95) %>%
    mutate(lower_ci = corr - ci,
           upper_ci = corr + ci) %>%
    ggplot(aes(x = orr_group, y = corr, group = social_cond,  fill = social_cond)) +
    geom_bar(color = "black",
              position = position_dodge(),
              width = 0.7,
              stat = "summary",
    ...
    labs(
        # title = "Accuracy Across All Time Points",
         x = "Social Condition",  # Change x-axis label
         y = "Accuracy (%)") +
    coord_cartesian(ylim = c(0.5, 0.94))  +
    styling +
    theme(axis.title.x = element_blank())

ggsave(paste0(output_path, "acc_by_orrTD.png"), bg="transparent", width =55, height = 20, units = "cm")
...
# Plot across time and social condition accuracy in TD
```{r, include = F}
library(ggplot2)
cbPalette <- c("#a6cee3", "#1f78b4", "#b2df8a", "#66c2a5", "#fc8d62", "#8da0cb", "#1b9e77", "#d95f02", "#7570b3")
...
plot_across_time <-
  function(word_class1, word_class2, group_name, df) {
  allRe %>%
    filter(word_type == word_class1 | word_type == word_class2) %>%
    filter(group == group_name) %>%
    filter(!is.na(corr)) %>%
    filter(!is.na(test_three)) %>%
    group_by(Participant.Public.ID, social_cond, test_three) %>%
    summarySEwithin(., measurevar = "corr", betweenvars = c("group"),  withinvars = c("social_cond", "test_three"), idvar= "Participant.Public.ID", na.rm=T, conf.interval=.95) %>%
    mutate(test_three = factor(test_three, levels=c("immediate", "attainment", "retention"))) %>%
    mutate(social_cond = dplyr::recode(social_cond, Direct = "Direct Address"),
           test_three = dplyr::recode(test_three, immediate = "0 min", attainment = "Same-Day", retention = "One-Week")) %>%
    mutate(lower_ci = corr - ci,
           upper_ci = corr + ci) %>%
    ggplot(aes(x = test_three, y = corr, group = social_cond,  fill = social_cond)) +
    scale_fill_manual(values = c(cbPalette[2], cbPalette[8])) +
    scale_color_manual(values = c(cbPalette[2], cbPalette[8])) +
    geom_bar(color = "black",
              position = position_dodge(),
              width = 0.7,
              stat = "summary",
...
```{r}
plot_across_time("noun", "noun", "TD", allRe)
ggsave(paste0(output_path, "meanAcc_noun_across_time_TD.png"), bg="transparent", width = 25, height = 15, units = "cm")

plot_across_time("verb", "verb", "TD", allRe)
ggsave(paste0(output_path, "meanAcc_verb_across_time_TD.png"), bg="transparent", width = 25, height = 15, units = "cm")
```


# Plot Attainment vs. Retention in TD
```{r}
allRe %>%
    filter(group == "TD") %>%
    filter(!is.na(corr)) %>%
    filter(!is.na(test_three)) %>%
    group_by(Participant.Public.ID, group, social_cond, test_three) %>%
   summarySEwithin(., measurevar = "corr", betweenvars = c("group"),  withinvars = c("social_cond", "test_three"), idvar= "Participant.Public.ID", na.rm=T, conf.interval=.95) %>%
   mutate(lower_ci = corr - ci,
           upper_ci = corr + ci) %>%
    mutate(test_three = factor(test_three, levels=c("immediate", "attainment", "retention"))) %>%
...
ggsave(paste0(output_path, "meanAcc_all_across_timeTD.png"), bg="transparent", width = 35, height = 15, units = "cm")
```




# Plot Modality and Bilingual Groups in Adults
```{r, include = F, eval = F}
adult_demo[which(as.numeric(adult_demo$short_id) < 900), "modality"] <- "auditory"
adult_demo[which(as.numeric(adult_demo$short_id) > 899), "modality"] <- "visual"
allReAd_demo <- merge(allRe, adult_demo[,c("record_id", "Bilingual", "modality")], by.x = "Participant.Public.ID", by.y = "record_id", all.x = T)

allReAd_demo %>%
    filter(modality == "visual") %>%
    filter(word_type == "verb" | word_type == "noun") %>%
    filter(group == "adult") %>%
    filter(!is.na(corr)) %>%
    filter(!is.na(test_three)) %>%
    group_by(Participant.Public.ID, social_cond, test_time, word_type, modality) %>%
    summarySEwithin(., measurevar = "corr", betweenvars = c("group", "modality"),  withinvars = c("social_cond", "word_type", "test_time"), idvar= "Participant.Public.ID", na.rm=T, conf.interval=.95) %>%
    mutate(test_time = factor(test_time, levels=c("immediate", "attainment", "short_retention", "retention"))) %>%
    mutate(social_cond = dplyr::recode(social_cond, Direct = "Direct Address"),
           modality = dplyr::recode(modality, auditory = "Auditory", visual = "Visual"),
           word_type = dplyr::recode(word_type, noun = "Noun", verb = "Verb"),
           test_time = dplyr::recode(test_time, immediate = "0 min", attainment = "10 min", short_retention = "60 min", retention = "1 week")) %>%
    mutate(lower_ci = corr - ci,
           upper_ci = corr + ci) %>%
    ggplot(aes(x = test_time, y = corr, group = social_cond,  fill = social_cond)) +
    # scale_fill_manual(values = c(cbPalette[2], cbPalette[8])) +
    # scale_color_manual(values = c(cbPalette[2], cbPalette[8])) +
...
ggsave(paste0(output_path, "meanAcc_visual_noun_verb_adult_red.png"), bg="transparent", width = 35, height = 15, units = "cm")
```
...
# Plot Immediate learning
```{r}
library(ggpattern)

plot_immediate <-
  function(df, testTime) {
  df %>%
    # filter(word_type == word_class1 | word_type == word_class2) %>%
    # filter(group == group_name) %>%
    filter(!is.na(corr)) %>%
    # filter(!is.na(test_three)) %>%
    filter(test_three %in% testTime) %>%
    arrange(group) %>%  # First sort by val. This sort the dataframe but NOT the factor levels
    mutate(group = factor(group, levels=c("TD", "adult"))) %>%
    mutate(social_cond = dplyr::recode(social_cond, Direct = "Direct Address"),
    ...
    group_by(Participant.Public.ID, group, social_cond, test_three) %>%
    dplyr::summarise(mean = mean(corr, na.rm = TRUE)) %>%
    summarySEwithin(., measurevar = "mean", betweenvars = "group",  withinvars = c("social_cond", "test_three"), idvar= "Participant.Public.ID", na.rm=T, conf.interval=.95) %>%
    ...
    guides(pattern = guide_legend(title="Social Condition", color = "black", override.aes = list(fill = c("white", "white")))) +
  # scale_pattern_spacing_discrete(range = c(0.025, 0.05)) + 
    scale_fill_manual(values = c(cbPalette[1], cbPalette[8])) +
    guides(fill = guide_legend(title = "Group", color = "black", override.aes = list(pattern = c("none", "none")))) +
   ...
   # guides(fill = guide_legend(title="Social Condition")) +
    ...
    geom_hline(yintercept = 0.25, size = 0.5, color = "black", linetype = 2) 
    # geom_text(aes(x = 0.525, y = 0.24, label = paste0("chance", "-level")), size = 3.8, face = "bold") 
}
```


```{r}
plot_immediate(meanAcc, "immediate")

ggsave(paste0(output_path, "imm_across_TDAdult.png"), bg="transparent", width = 20, height = 10, units = "cm")
```


# Plot Mean of Attainment and Retention in TD
```{r}
library(ggpattern)

unique(allRe$test_time)
...
allRe %>%
    ...
    # filter(!is.na(test_three)) %>%
    filter(test_three == "attainment") %>%
    # filter(test_time == "short_retention") %>%
    mutate(social_cond = dplyr::recode(social_cond, Direct = "Direct Address"),
           test_three = dplyr::recode(test_three, immediate = "Immediate Test", attainment = "Short-Term Retention", retention = "Long-Term Retention")) %>%
    group_by(Participant.Public.ID, group, social_cond) %>%
    dplyr::summarise(mean = mean(corr, na.rm = TRUE)) %>%
    summarySEwithin(., measurevar = "mean", betweenvars = "group",  withinvars = c("social_cond"), idvar= "Participant.Public.ID", na.rm=T, conf.interval=.95) %>%
    mutate(lower_ci = mean - ci,
           upper_ci = mean + ci) %>%
    ggplot(aes(x = group, y = mean, pattern = social_cond,  fill = group)) +
    geom_col_pattern(aes(pattern = social_cond),
    ...
    guides(pattern = guide_legend(title="Social Condition", color = "black", override.aes = list(fill = c("white", "white")))) +
  # scale_pattern_spacing_discrete(range = c(0.025, 0.05)) + 
    scale_fill_manual(values = c(cbPalette[8])) +
    guides(fill = guide_legend(title = "Group", color = "black", override.aes = list(pattern = c("none")))) +
    scale_color_manual(values = c("black")) +
    geom_errorbar(aes(ymin = mean - se, ymax = mean + se),
                  width = .1,
                  position = position_dodge(width = 0.7)) +
    geom_point(color = "black",
               ...
               stat = "summary",
               fun = "mean", show.legend =  F) +
     labs(y = "Retention Accuracy (%)") +
      # labs(title = paste("Accuracy in", word_class1, word_class2)) +
    styling +
    coord_cartesian(ylim = c(0.6, 0.75)) +
    # geom_text(size = 11, color = "black", aes(x = 1, y = 0.71, label = paste0("*"))) +
    geom_text(size = 11, color = "black", aes(x = 1, y = 0.73, label = paste0("†"))) +
    geom_segment(aes(x = 0.75, xend = 1.25, y = 0.7, yend = 0.7))

ggsave(paste0(output_path, "retAdult_attlr.png"), bg="transparent", width = 20, height = 15, units = "cm")    
```

# Plot Mean of Attainment and Retention across groups
```{r}
allRe %>%
    # filter(word_type == word_class1 | word_type == word_class2) %>%
    filter(!is.na(corr)) %>%
    # filter(!is.na(test_three)) %>%
    filter(test_time == "attainment" | test_time == "short_retention") %>%
    arrange(group) %>%  # First sort by val. This sort the dataframe but NOT the factor levels
    mutate(group = factor(group, levels=c("TD", "adult"))) %>%
    mutate(social_cond = dplyr::recode(social_cond, Direct = "Direct Address"),
           test_three = dplyr::recode(test_three, immediate = "Immediate Test", attainment = "Short-Term Retention", retention = "Long-Term Retention")) %>%
  ...
  # scale_pattern_spacing_discrete(range = c(0.025, 0.05)) + 
    ...
    geom_point(color = "black",
               ...
               fun = "mean", show.legend =  F) +
    labs(y = "Accuracy (%)") +
    labs(title = "Retention Test") +
    styling +
    coord_cartesian(ylim = c(0.6, 1)) +
    geom_segment(aes(x = 0.75, xend = 1.25, y = 0.73, yend = 0.73)) +
    geom_segment(aes(x = 1.75, xend = 2.25, y = 0.7, yend = 0.7)) +
    geom_text(size = 7, color = "black",aes(x = 2, y = 0.73, label = paste0("†"))) +
    geom_segment(aes(x = 1, xend = 2, y = 0.76, yend = 0.76)) +
    geom_segment(aes(x = 1, xend = 1, y = 0.76, yend = 0.75)) +
    geom_segment(aes(x = 2, xend = 2, y = 0.76, yend = 0.75)) +
    geom_segment(aes(x = 1.5, xend = 1.5, y = 0.76, yend = 0.765)) +
    geom_text(size = 11, color = "black",aes(x = 1.5, y = 0.77, label = paste0("*")))

ggsave(paste0(output_path, "retTDAdult_attlr.png"), bg="transparent", width = 20, height = 10, units = "cm")  
```


# Plot Attainment vs. Retention across groups
```{r}
allRe %>%
    # filter(group == group_name) %>%
    filter(!is.na(corr)) %>%
    ...
    # filter(test_time == "retention" | test_time == "attainment") %>%
    arrange(group) %>%  # First sort by val. This sort the dataframe but NOT the factor levels
    mutate(group = factor(group, levels=c("TD", "adult"))) %>%
    mutate(social_cond = dplyr::recode(social_cond, Direct = "Direct Address"),
   ...
   # guides(fill = guide_legend(title="Social Condition")) +
    ...
    geom_segment(data = data.frame(test_three = "Long-Term Retention", group = "adult", social_cond = "Direct Address"),
                 aes(x = 0.75, xend = 1.25, y = 0.7, yend = 0.7)) +
    # geom_text(aes(x = 1, y = 0.805, label = paste0("*")), size = 11, color = "black") +
    geom_segment(data = data.frame(test_three = "Long-Term Retention", group = "adult", social_cond = "Direct Address"),
                 aes(x = 1.75, xend = 2.25, y = 0.7, yend = 0.7)) +
    geom_segment(data = data.frame(test_three = "Long-Term Retention", group = "adult", social_cond = "Direct Address"),
                 aes(x = 1, xend = 2, y = 0.73, yend = 0.73)) +
    geom_segment(data = data.frame(test_three = "Long-Term Retention", group = "adult", social_cond = "Direct Address"),
                 aes(x = 1, xend = 1, y = 0.73, yend = 0.72)) +
    geom_segment(data = data.frame(test_three = "Long-Term Retention", group = "adult", social_cond = "Direct Address"),
                 aes(x = 2, xend = 2, y = 0.73, yend = 0.72)) +
    geom_segment(data = data.frame(test_three = "Long-Term Retention", group = "adult", social_cond = "Direct Address"),
                 aes(x = 1.5, xend = 1.5, y = 0.73, yend = 0.735)) +
    # geom_text(size = 5, color = "black", 
    #           data = data.frame(test_three = "Long-Term Retention", group = "adult", social_cond = "Direct Address"),
    #           aes(x = 2, y = 0.71, label = paste0("?"))) +
    geom_text(size = 7, color = "black", 
              data = data.frame(test_three = "Long-Term Retention", group = "adult", social_cond = "Direct Address"),
              aes(x = 1.5, y = 0.76, label = paste0("†"))) +
  geom_text(size = 11, color = "black", 
              data = data.frame(test_three = "Short-Term Retention", group = "adult", social_cond = "Direct Address"),
              aes(x = 1.5, y = 0.78, label = paste0("*"))) +
  geom_segment(data = data.frame(test_three = "Short-Term Retention", group = "adult", social_cond = "Direct Address"),
                 aes(x = 1, xend = 2, y = 0.775, yend = 0.775))
    # geom_text(aes(x = 1.5, y = 0.855, label = paste0("†")), size = 7, color = "black")
```


```{r}
ggsave(paste0(output_path, "att_ret_by_timeTDAdult.png"), bg="transparent", width = 30, height = 10, units = "cm")
```
...
## Read in Gaze Data
```{r}
gaze_child <- read.csv("/Users/jojohu/Documents/Splash/beh_analysis/results/eye_gaze/gaze_child.csv")
```



## Plot Eye Gaze Proportions
```{r}
labsLook <- c("Unclear", "Speaker", "Listener")
names(labsLook) <- c("cannot_tell", "speaker", "listener")

gaze_child %>%
  filter(look_object == "speaker" | look_object == "listener" | look_object == "cannot_tell") %>%
  arrange(group) %>%  # First sort by val. This sort the dataframe but NOT the factor levels
  mutate(group = factor(group, levels=c("TD", "adult")),
         look_object = factor(look_object, levels=c("speaker", "listener", "cannot_tell"))) %>%
  mutate(social_cond = dplyr::recode(social_cond, Direct = "Direct Address")) %>%
  group_by(part_id, group, social_cond, look_object) %>%
  dplyr::summarise(mean_freq = mean(freq)) %>%
  summarySEwithin(., measurevar = "mean_freq", betweenvars = "group",  withinvars = c("social_cond", "look_object"), idvar= "part_id", na.rm=T, conf.interval=.95) %>%
  mutate(lower_ci = mean_freq - ci,
         upper_ci = mean_freq + ci) %>%
  arrange(group, social_cond, look_object) %>%
  ggplot(aes(x = group, y = mean_freq, pattern = social_cond,  fill = group)) +
    geom_col_pattern(aes(pattern = social_cond),
                     stat = "summary",
                     fun = "mean",
                     width = 0.7,
    ...
    geom_point(color = "black",
               ...
               fun = "mean",
               show.legend = FALSE) +
  facet_grid(~look_object, labeller = (labeller(look_object = labsLook))) +
  labs(y = "Mean Proportion of Looks (%)") +
  styling +
  theme(axis.title.x = element_blank())


ggsave(paste0(output_path, "looks_tdAdult.png"), bg="transparent", width = 40, height = 15, units = "cm")
```


## Accuracy Correlation with Eye Gaze
```{r}
gaze_child$word_type <- str_extract(tolower(gaze_child$trial_type), "(noun|verb)")

gazeCM <- 
  gaze_child %>%
    group_by(part_id, group, word_type, social_cond, look_object) %>%
    dplyr::summarise(mean_freq = mean(freq))

gazeCM <- 
  merge(meanAcc, gazeCM, 
        by.x = c("Participant.Public.ID", "group", "social_cond", "word_type"), 
        by.y = c("part_id", "group", "social_cond", "word_type"), all.y = T)
```


## Plot Accuracy Correlation with Eye Gaze 
```{r}
plot_gazeAcc <- 
  function(groupName, time) {
    gazeCM %>%
      filter(group == groupName) %>%
      filter(test_three == time) %>%
      filter(!is.na(corr)) %>%
      filter(!is.na(test_three)) %>%
      filter(look_object != "center") %>%
      arrange(group, social_cond) %>%
      dplyr::mutate(social_cond = dplyr::recode(social_cond, "front" = "Direct", "side" = "Overhearing"),
                    corr = as.numeric(as.character(corr))) %>%
      ggplot(aes(x = mean_freq, y = corr)) +
      geom_point(
        aes(color = social_cond),
        stat = "identity",
        size = 3,
      ...
      facet_wrap(~group+look_object+test_three,  labeller = (labeller(word_type = labsWord))) +
      geom_smooth(method='lm', formula= y~x, aes(color = social_cond), se = F) +
      guides(fill = guide_legend(title = "Social Condition")) +
      labs(title = "Recall Accuracy",
           x = "Mean Proportion of Looks (%)",  # Change x-axis label
           y = "Accuracy (%)") +
      styling
} 


plot_gazeAcc("TD", "immediate")
plot_gazeAcc("TD", "attainment")
plot_gazeAcc("TD", "retention")
```
...
gazeCM %>%
      filter(!is.na(corr)) %>%
      filter(test_three == "retention") %>%
      filter(look_object == "speaker") %>%
      filter(social_cond == "Overhearing") %>%
      filter(group == "TD") %>%
      arrange(group, social_cond) %>%
      # group_by(Participant.Public.ID, group, social_cond) %>%
      # dplyr::summarise(corr = mean(corr, na.rm = T),
      #                  mean_freq = mean(mean_freq, na.rm = T)) %>%
      dplyr::mutate(social_cond = dplyr::recode(social_cond, "front" = "Direct", "side" = "Overhearing"),
                    corr = as.numeric(as.character(corr))) %>%
      ggplot(aes(x = mean_freq, y = corr)) +
      ...
      # facet_wrap(~test_three,  labeller = (labeller(word_type = labsWord))) +
      geom_smooth(method='lm', formula= y~x, color = "#d95f02", se = F) +
      guides(fill = guide_legend(title = "Social Condition")) +
      labs(x = "Mean Proportion of Looks to the Speaker (%)",  # Change x-axis label
           y = "Long-Term Retention Accuracy (%)") +
     # stat_cor(method = "spearman", label.x = 0.2,
     #         inherit.aes = T, color = "black", size = 7) +
      # geom_label(aes(x = 0.4, y = 0.9), label = expression(paste(italic("rho"), " = 0.42*")), 
      #             size = 7, color = "black", label.size = NA) +
      coord_cartesian(xlim = c(0.2, 1)) +
      theme(legend.position = c(0.89, 0.15)) +
      styling
            
ggsave("/Users/jojohu/Documents/Splash/beh_analysis/results/gaze_acc_oh_td.png", width = 15, height = 15, units = "cm")
```
...
meanAcc$group <- factor(meanAcc$group, levels = c("adult", "TD"))
...
meanAccad <- meanAcc[which(meanAcc$group == "adult"),]
...
# Vocabulary Analysis
```{r}
vocab_m1 <- lm(corr ~ social_cond*scaled_age, data = meanAccTD,
               contrasts = list(
                   social_cond = c(-0.5, 0.5),
                   word_type = c(-0.5, 0.5),
                   test_three = three_helmert
...
orr_m1 <- lm(corr ~ social_cond*test_three + scaled_age + pvt_raw_score + orr_raw_score, data = meanAccTD,
               contrasts = list(
                   social_cond = c(-0.5, 0.5),
                   word_type = c(-0.5, 0.5),
                   test_three = three_helmert
...
ggsave("results/pvt_acc_immTD.png", width = 20, height = 15, units="cm")
```
...
ggsave("results/mite_acc_attTD.png", width = 30, height = 15, units = "cm")
```
